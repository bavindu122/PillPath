...
import { Package, DollarSign, Users, Box, Wallet } from 'lucide-react';
import { useNavigate } from 'react-router-dom';

const DashboardOverview = ({ data, isLoading = false }) => {
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8 mb-6">
        {[1, 2, 3, 4, 5].map((i) => (
          <div
            key={i}
            className="border border-gray-200 bg-gray-50 rounded-2xl px-8 py-8 min-w-[220px] min-h-[180px] animate-pulse"
          >
  ...
  const dashboardStats = [
    ...
    { type: 'Digital Wallet', count: data.digitalWallet.value, description: data.digitalWallet.description },
  ];
