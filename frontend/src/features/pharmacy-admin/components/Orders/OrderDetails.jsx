...
const OrderDetails = ({ order }) => {
  ...
  // ✅ NEW: Determine item type based on medicine/item code or name
  const getItemType = (item) => {
    // Check if item has explicit category/type
    if (item.category) {
    ...
    // Check medicine code/ID if available
    const itemCode = (item.medicineCode || item.itemCode || item.code || item.id || '').toString().toUpperCase();
    if (itemCode.startsWith('PMED') || itemCode.startsWith('PRESC')) {
      return 'prescription';
    }
    if (itemCode.startsWith('OTC')) {
      return 'otc';
    }
    
    // Check medicine name for OTC indicators
    const itemName = (item.name || item.medicineName || '').toString().toUpperCase();
    if (itemName.includes('OTC')) {
      return 'otc';
    }
    
    // Since the overall order type is determined by order code,
    // we can use that as a fallback for all items in the order
  ...
  // ✅ UPDATED: Determine order type from code first, then fallback to orderType field
  const determinedOrderType = getOrderTypeFromCode(order.orderCode || order.orderNumber || order.id) || 
                               (order.orderType || order.type);
  const orderTypeInfo = getOrderTypeInfo(determinedOrderType);
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {(order.items || []).map((item, index) => {
                // ✅ UPDATED: Get accurate item type
                ...
                return (
                  <tr key={`${item.id}-${index}`} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      ...
                      >
                        {itemType === "prescription" ? "Prescription" : "OTC"}
                      </span>
                    </td>
