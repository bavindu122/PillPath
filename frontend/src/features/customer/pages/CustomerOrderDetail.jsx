...
export default function CustomerOrderDetail() {
  ...
  React.useEffect(() => {
    ...
    OrdersService.getOrder(orderCode)
      ...
      .catch(e => { if (!cancelled) setError(e.message || 'Failed to load order'); })
        ...
        {order && !loading && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-xl border border-white/20 p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-white/80 text-sm">
                <div>
                  ...
                  <div className="font-medium">{order.prescriptionCode || '—'}</div>
                </div>
                <div>
              ...
              {order.totals && (
                <div className="mt-6 flex flex-wrap gap-6 text-white/90">
                  <div>
                    ...
                    <div className="font-semibold">{totalCurrency} {order.totals.subtotal?.toFixed?.(2) ?? order.totals.subtotal}</div>
                  </div>
                  <div>
                    <div className="text-white/50 text-xs">Discount</div>
                    <div className="font-semibold">{totalCurrency} {order.totals.discount?.toFixed?.(2) ?? order.totals.discount}</div>
                  </div>
                  <div>
                    <div className="text-white/50 text-xs">Tax</div>
                    <div className="font-semibold">{totalCurrency} {order.totals.tax?.toFixed?.(2) ?? order.totals.tax}</div>
                  </div>
                  <div>
                    <div className="text-white/50 text-xs">Shipping</div>
                    <div className="font-semibold">{totalCurrency} {order.totals.shipping?.toFixed?.(2) ?? order.totals.shipping}</div>
                  </div>
                  <div>
                    <div className="text-white/50 text-xs">Total</div>
                    <div className="font-bold text-lg">{totalCurrency} {order.totals.total?.toFixed?.(2) ?? order.totals.total}</div>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="space-y-6">
              <h2 className="text-xl font-semibold text-white">Pharmacy Orders</h2>
              {(order.pharmacyOrders || order.pharmacies || []).map((po, idx) => {
                const status = po.status || po.orderStatus || 'PENDING';
                return (
                  <div key={idx} className="bg-white/10 backdrop-blur-sm rounded-xl border border-white/20 p-5">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center space-x-3">
                        <Store className="h-5 w-5 text-white/60" />
                        <div>
                          <div className="text-white font-medium">{po.pharmacyName || `Pharmacy #${po.pharmacyId || po.id || idx+1}`}</div>
                          {po.pickupCode && (
                            <div className="text-xs text-white/50">Pickup Code: {po.pickupCode}</div>
                          )}
                        </div>
                      </div>
                      <div className={`text-sm font-semibold ${statusColors[status] || 'text-white/70'}`}>{status.replace(/_/g,' ')}</div>
                    </div>
                    <div className="space-y-2">
                      {(po.items || []).map((it,i) => (
                        <div key={i} className="flex justify-between items-center bg-white/5 rounded-lg px-3 py-2 border border-white/10">
                          <div>
                            <div className="text-white text-sm font-medium">{it.medicineName || it.name || `Item ${i+1}`}</div>
                            <div className="text-white/50 text-xs">Qty: {it.quantity || 1}</div>
                          </div>
                          <div className="text-white font-semibold text-sm">
                            {totalCurrency} { (it.unitPrice ?? it.price ?? 0).toFixed ? (it.unitPrice ?? it.price ?? 0).toFixed(2) : (it.unitPrice ?? it.price ?? 0) }
                          </div>
                        </div>
                      ))}
                      {(!po.items || po.items.length === 0) && (
                        <div className="text-white/50 text-sm italic">No items</div>
                      )}
                    </div>
                  </div>
                );
