...
const EditProfileModal = ({ isOpen, onClose, selectedProfile, onSuccess }) => {
  const { user, loading, updateUser } = useAuth();
  
  // Determine if we're editing a family member or the main user
  const isFamilyMember = selectedProfile && selectedProfile.id !== 0 && selectedProfile.relation !== "Me";
  const profileToEdit = isFamilyMember ? selectedProfile : user;
  ...
  useEffect(() => {
    if (isFamilyMember && selectedProfile?.profilePicture) {
      setProfileImagePreview(selectedProfile.profilePicture);
    } else if (user?.profilePictureUrl) {
      setProfileImagePreview(user.profilePictureUrl);
    }
  }, [user, selectedProfile, isFamilyMember]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isFamilyMember) {
        // For family members, we don't upload profile picture separately
        // Prepare data for backend
        console.log('=== PREPARING FAMILY MEMBER DATA ===');
        ...
        const memberData = {
          ...
          relation: formData.relation || selectedProfile.relation,
          age: formData.age ? parseInt(formData.age) : selectedProfile.age,
          profilePicture: profileImagePreview || selectedProfile.profilePicture,
          email: formData.email || selectedProfile.email,
          phone: formData.phone || selectedProfile.phone,
          bloodType: formData.bloodType || selectedProfile.bloodType,
          ...
          medicalConditions: formData.medicalConditions
            ...
            : [],
          currentMedications: formData.currentMedications || selectedProfile.currentMedications || [],
          lastPrescriptionDate: selectedProfile.lastPrescriptionDate || null,
          activePrescriptions: selectedProfile.activePrescriptions || 0,
          totalPrescriptions: selectedProfile.totalPrescriptions || 0,
        };

        console.log('=== SENDING TO BACKEND ===');
        console.log('Member ID:', selectedProfile.id);
        console.log('Member Data:', JSON.stringify(memberData, null, 2));
        console.log('==========================');

        const result = await FamilyMemberService.updateFamilyMember(selectedProfile.id, memberData);
        console.log('=== BACKEND RESPONSE ===');
        console.log('Family member updated:', result);
        console.log('========================');
        
        // Show success message
        alert('Family member profile updated successfully!');
        ...
        // Handle main user profile update
  ...
  const handleClose = () => {
    resetForm();
    const currentProfilePicture = isFamilyMember 
      ? selectedProfile?.profilePicture 
      : user?.profilePictureUrl;
    setProfileImagePreview(currentProfilePicture || null);
    setSelectedFile(null);
    onClose();
  };
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-white/20">
          <h2 className="text-2xl font-bold text-white flex items-center gap-3">
            ...
            {isFamilyMember ? `Edit ${selectedProfile.name}'s Profile` : 'Edit Profile'}
          </h2>
          <button
            onClick={handleClose}
        ...
        {loading ? (
          ...
          <>
            ...
            <ModalScrollContainer>
              {/* Profile Picture Section - Only show for main user */}
              {!isFamilyMember && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  {isFamilyMember ? (
                    <>
                      <FormField label="Full Name" field="name" />
                      ...
                      <FormField label="Phone Number" field="phone" type="tel" />
                ...
                {/* Contact & Address Information OR Medical Info for Family Members */}
                <div className="space-y-4">
                  {isFamilyMember ? (
                    <>
                      <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
                      ...
                      />
                      <FormField
                        key="familyMember-medicalConditions"
                ...
                {/* Medical Information - Only for main user */}
                {!isFamilyMember && (
                  <div className="space-y-4">
                    <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
                      <div className="w-2 h-6 bg-yellow-500 rounded-full"></div>
                    ...
                    />
                    <FormField
                      key="mainUser-medicalConditions"
                      label="Medical Conditions"
                      field="medicalConditions"
                      type="textarea"
                      placeholder="List any chronic conditions (separate with commas)..."
                ...
                {/* Emergency & Insurance - Only for main user */}
                {!isFamilyMember && (
                  <div className="space-y-4">
                    <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
                      <div className="w-2 h-6 bg-yellow-500 rounded-full"></div>
