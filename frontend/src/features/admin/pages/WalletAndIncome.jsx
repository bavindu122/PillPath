...
import { DollarSign, TrendingUp, TrendingDown, Store, Search, Filter, Calendar, Download, Wallet, Eye, CheckCircle, XCircle, Upload, CreditCard, Hand,HandCoins,Handshake } from 'lucide-react';
...
const mockInitialSalesTransactions = [
  ...
  { id: 'ORD001', date: '2023-01-10', sender: 'Customer A', receiver: 'City Pharmacy', amount: 100.00, type: 'Order Payment', pharmacyName: 'City Pharmacy', paymentType: 'Online' },
  { id: 'ORD002', date: '2023-01-12', sender: 'Customer B', receiver: 'Health Hub', amount: 50.00, type: 'Order Payment', pharmacyName: 'Health Hub', paymentType: 'On-Hand' },
  { id: 'ORD003', date: '2023-02-05', sender: 'Customer C', receiver: 'MediCare Drugstore', amount: 75.00, type: 'Order Payment', pharmacyName: 'MediCare Drugstore', paymentType: 'Online' },
  { id: 'ORD004', date: '2023-02-15', sender: 'Customer D', receiver: 'City Pharmacy', amount: 120.00, type: 'Order Payment', pharmacyName: 'City Pharmacy', paymentType: 'On-Hand' },
  { id: 'ORD005', date: '2023-03-01', sender: 'Customer E', receiver: 'Quick Meds', amount: 90.00, type: 'Order Payment', pharmacyName: 'Quick Meds', paymentType: 'Online' },
  { id: 'ORD006', date: '2023-03-20', sender: 'Customer F', receiver: 'Health Hub', amount: 65.00, type: 'Order Payment', pharmacyName: 'Health Hub', paymentType: 'On-Hand' },
  { id: 'ORD007', date: '2023-04-08', sender: 'Customer G', receiver: 'City Pharmacy', amount: 110.00, type: 'Order Payment', pharmacyName: 'City Pharmacy', paymentType: 'Online' },
  { id: 'ORD008', date: '2023-04-25', sender: 'Customer H', receiver: 'MediCare Drugstore', amount: 80.00, type: 'Order Payment', pharmacyName: 'MediCare Drugstore', paymentType: 'On-Hand' },
  { id: 'ORD009', date: '2023-05-02', sender: 'Customer I', receiver: 'Quick Meds', amount: 150.00, type: 'Order Payment', pharmacyName: 'Quick Meds', paymentType: 'Online' },
  { id: 'ORD010', date: '2023-05-19', sender: 'Customer J', receiver: 'Health Hub', amount: 70.00, type: 'Order Payment', pharmacyName: 'Health Hub', paymentType: 'On-Hand' },
  { id: 'ORD011', date: '2023-06-03', sender: 'Customer K', receiver: 'City Pharmacy', amount: 95.00, type: 'Order Payment', pharmacyName: 'City Pharmacy', paymentType: 'Online' },
  { id: 'ORD012', date: '2023-06-28', sender: 'Customer L', receiver: 'MediCare Drugstore', amount: 130.00, type: 'Order Payment', pharmacyName: 'MediCare Drugstore', paymentType: 'On-Hand' },
  { id: 'ORD013', date: '2023-07-07', sender: 'Customer M', receiver: 'Quick Meds', amount: 88.00, type: 'Order Payment', pharmacyName: 'Quick Meds', paymentType: 'Online' },
  { id: 'ORD014', date: '2023-07-14', sender: 'Customer N', receiver: 'Health Hub', amount: 105.00, type: 'Order Payment', pharmacyName: 'Health Hub', paymentType: 'On-Hand' },
  { id: 'ORD015', date: '2023-07-20', sender: 'Customer O', receiver: 'City Pharmacy', amount: 60.00, type: 'Order Payment', pharmacyName: 'City Pharmacy', paymentType: 'Online' },
  { id: 'ORD016', date: '2023-07-25', sender: 'Customer P', receiver: 'MediCare Drugstore', amount: 70.00, type: 'Order Payment', pharmacyName: 'MediCare Drugstore', paymentType: 'On-Hand' },
  { id: 'ORD017', date: '2023-08-01', sender: 'Customer Q', receiver: 'Health Hub', amount: 115.00, type: 'Order Payment', pharmacyName: 'Health Hub', paymentType: 'Online' },
  { id: 'ORD018', date: '2023-08-05', sender: 'Customer R', receiver: 'Quick Meds', amount: 85.00, type: 'Order Payment', pharmacyName: 'Quick Meds', paymentType: 'On-Hand' },
];

const processFinancialData = (transactions, commissionRate = 0.10) => {
  const processedData = [];
  ...
  transactions.forEach(tx => {
    if (tx.type === 'Order Payment') {
      ...
      const transactionMonth = new Date(tx.date).toLocaleString('en-US', { month: '2-digit', year: 'numeric' }); // MM/YYYY
...
const WalletAndIncome = () => {
  ...
  const { processedData, uniquePharmacies } = useMemo(() => processFinancialData(mockInitialSalesTransactions), []);
  ...
  const [isPharmacyDetailsModalOpen, setIsPharmacyDetailsModalOpen] = useState(false);
  const [selectedPharmacy, setSelectedPharmacy] = useState(null);

  // State for receipt upload modal
  const [isReceiptUploadModalOpen, setIsReceiptUploadModalOpen] = useState(false);
  ...
  const customerPayments = filteredTransactions.filter(tx => tx.type === 'Order Payment');
  const indexOfLastCustomerPayment = currentPage * transactionsPerPage;
  const indexOfFirstCustomerPayment = indexOfLastCustomerPayment - transactionsPerPage;
  const currentCustomerPayments = customerPayments.slice(indexOfFirstCustomerPayment, indexOfLastCustomerPayment);
  const totalCustomerPaymentPages = Math.ceil(customerPayments.length / transactionsPerPage);
  ...
  const currentWalletBalance = totalOnlinePaymentIncome - totalPayoutsToPharmacies;
  ...
  const topPharmaciesData = Object.keys(commissionByPharmacy)
    .map(name => ({ name, commission: parseFloat(commissionByPharmacy[name].toFixed(2)) }))
    .sort((a, b) => b.commission - a.commission)
    .slice(0, 5);

  // Prepare data for Monthly Online vs. On-Hand Payments Chart
  const paymentsByMonth = filteredTransactions.reduce((acc, tx) => {
    if (tx.type === 'Order Payment') { // Only consider customer payments for this chart
  ...
  const years = ['All', ...new Set(transactions.map(tx => new Date(tx.date).getFullYear().toString()))].sort();
  ...
  const getPharmacyWalletSummary = (pharmacyName) => {
    const pharmacyTransactions = transactions.filter(tx => tx.pharmacyName === pharmacyName);

    const totalPaidCommissions = pharmacyTransactions
      .filter(tx => tx.type === 'Commission Payment' && tx.commissionStatus === 'Paid')
      .reduce((sum, tx) => sum + tx.amount, 0);
    ...
    if (payoutToMark) {
      setTransactions(prevTransactions =>
        prevTransactions.map(tx =>
          tx.id === payoutToMark.id ? { ...tx, payoutStatus: 'Paid', receiptUrl: receiptUrlInput } : tx
        )
      );
      console.log(`Payout ${payoutToMark.id} marked as Paid with receipt: ${receiptUrlInput}`);
  ...
  const getPharmacyDetailedTransactions = (pharmacyName) => {
    const customerPayments = transactions.filter(tx =>
      tx.type === 'Order Payment' && tx.receiver === pharmacyName
    );

    const payoutsFromSystem = transactions.filter(tx =>
      tx.type === 'Payout' && tx.receiver === pharmacyName
    );

    const commissionsDue = transactions.filter(tx =>
      tx.type === 'Commission Payment' && tx.sender === pharmacyName && tx.paymentType === 'On-Hand'
    );
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8 font-sans">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6 mb-10">
            <StatCard label="Total Revenue" value={`Rs. ${totalRevenue.toLocaleString()}`} icon={<HandCoins size={48} className="text-green-500" />} />
            <StatCard label="Total Online Payments Income" value={`Rs. ${totalOnlinePaymentIncome.toLocaleString()}`}  icon={<CreditCard size={48} className="text-pink-500" />} />
            <StatCard label="Total On-Hand Payments Income" value={`Rs. ${totalOnHandPaymentIncome.toLocaleString()}`}  icon={<Handshake size={48} className="text-purple-500" />} />
            <StatCard label="Current Wallet Balance" value={`Rs. ${currentWalletBalance.toLocaleString()}`}  icon={<Wallet size={48} className="text-blue-500" />} />
        </div>

        {/* Pharmacy Wallet Overview */}
      <div className="bg-white p-6 rounded-lg shadow-md mb-6">
        <h3 className="text-xl font-semibold text-gray-800 mb-4 flex items-center">
          <Store className="mr-2 text-gray-600" size={24} /> Pharmacy Wallet Overview
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {uniquePharmacies.map(pharmacyName => {
            const summary = getPharmacyWalletSummary(pharmacyName);
            return (
              <div key={pharmacyName} className="bg-gray-50 p-4 rounded-lg shadow-sm border border-gray-200">
                <h4 className="font-semibold text-lg text-gray-800 mb-2">{pharmacyName}</h4>
                <p className="text-sm text-gray-600">Commissions Paid: <span className="font-medium text-green-700">${summary.totalPaidCommissions}</span></p>
                <p className="text-sm text-gray-600">Payouts Received: <span className="font-medium text-blue-700">${summary.totalPayoutsReceived}</span></p>
                <button
      ...
      <div className="bg-white p-6 rounded-lg shadow-md mb-6">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <button
            onClick={() => handleExportData('PillPath_Sales', filteredTransactions)}
      ...
      <div className="bg-white p-6 rounded-lg shadow-md mb-6">
        <h3 className="text-xl font-semibold text-gray-800 mb-4">Monthly Online vs. On-Hand Payments</h3>
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={chartDataPayments} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
            ...
            <Legend />
            <Bar dataKey="online" fill="#4A90E2" name="Online Payments" radius={[5, 5, 0, 0]} />
            <Bar dataKey="onHand" fill="#9B59B6" name="On-Hand Payments" radius={[5, 5, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow-md mb-6">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={topPharmaciesData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
            <CartesianGrid strokeDasharray="3 3" />
            <XAxis dataKey="name" angle={-15} textAnchor="end" height={60} interval={0} />
            <YAxis />
            <Tooltip formatter={(value) => `$${value.toFixed(2)}`} />
            <Legend />
            <Bar dataKey="commission" fill="#82ca9d" name="Commission Paid" radius={[10, 10, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow-md overflow-x-auto mt-6">
        <h3 className="text-xl font-semibold text-gray-800 mb-4">Customer Payments & Commissions</h3>
            ...
            {currentCustomerPayments.length > 0 ? (
              currentCustomerPayments.map((tx) => (
                <tr key={tx.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{tx.sender}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{tx.receiver}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                      tx.paymentType === 'Online' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800'
                    }`}>
                      {tx.paymentType}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{tx.amount.toFixed(2)}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{tx.commissionAmount.toFixed(2)}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                      tx.receivedStatus === 'Received' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                    }`}>
                      {tx.receivedStatus}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    {tx.paymentType === 'On-Hand' && tx.receivedStatus === 'Unreceived' && (
                      <button
                        onClick={() => handleMarkPaymentReceivedStatus(tx.id, 'Received')}
                    ...
                    {tx.paymentType === 'On-Hand' && tx.receivedStatus === 'Received' && (
      ...
      {isPharmacyDetailsModalOpen && selectedPharmacy && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white p-8 rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <h3 className="text-2xl font-bold text-gray-800 mb-6">Financial Details for {selectedPharmacy}</h3>

            {/* Customer Payments to this Pharmacy */}
            <div className="mb-8">
              <h4 className="text-xl font-semibold text-gray-800 mb-3 flex items-center">
                <TrendingUp size={20} className="mr-2 text-green-600" /> Customer Payments Received by Pharmacy
              </h4>
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200 border border-gray-200 rounded-lg">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {getPharmacyDetailedTransactions(selectedPharmacy).customerPayments.length > 0 ? (
                      getPharmacyDetailedTransactions(selectedPharmacy).customerPayments.map(tx => (
                        <tr key={tx.id}>
                          ...
                          <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">{tx.sender}</td>
                          ...
                          <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">{tx.amount.toFixed(2)}</td>
                          <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">{tx.commissionAmount.toFixed(2)}</td>
                        </tr>
                      ))
                    ) : (
                      <tr>
                        <td colSpan="6" className="px-4 py-2 text-center text-sm text-gray-500">No customer payments found.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
            ...
            <div className="mb-8">
              <h4 className="text-xl font-semibold text-gray-800 mb-3 flex items-center">
                <TrendingDown size={20} className="mr-2 text-red-600" /> Payouts from PillPath
              </h4>
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200 border border-gray-200 rounded-lg">
                  <thead className="bg-gray-50">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {getPharmacyDetailedTransactions(selectedPharmacy).payoutsFromSystem.length > 0 ? (
                      getPharmacyDetailedTransactions(selectedPharmacy).payoutsFromSystem.map(tx => (
                        <tr key={tx.id}>
                          <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-900">{tx.id}</td>
                          <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">{tx.date}</td>
                          <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">{tx.payoutMonth}</td>
                          <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">{tx.amount.toFixed(2)}</td>
                          <td className="px-4 py-2 whitespace-nowrap text-sm">
                            <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                              tx.payoutStatus === 'Paid' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                            }`}>
                              {tx.payoutStatus}
                            </span>
                          </td>
                          <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">
                            {tx.receiptUrl ? (
                              <a href={tx.receiptUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">View Receipt</a>
                            ) : (
                              'N/A'
                            )}
                          </td>
                          <td className="px-4 py-2 whitespace-nowrap text-sm">
                            {tx.payoutStatus === 'Unpaid' && (
                              <button
                                onClick={() => handleMarkPayoutStatusInitiate(tx)}
                                className="text-green-600 hover:text-green-900 p-1 rounded-full hover:bg-green-100 transition-colors"
                                title="Mark as Paid"
                              >
                                <CheckCircle size={18} />
                      ...
                      <tr>
                        <td colSpan="7" className="px-4 py-2 text-center text-sm text-gray-500">No payouts from PillPath found.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
            ...
            <div className="mb-8">
              <h4 className="text-xl font-semibold text-gray-800 mb-3 flex items-center">
                <DollarSign size={20} className="mr-2 text-orange-600" /> Commissions Due (On-Hand Payments)
              </h4>
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200 border border-gray-200 rounded-lg">
                  <thead className="bg-gray-50">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {getPharmacyDetailedTransactions(selectedPharmacy).commissionsDue.length > 0 ? (
                      getPharmacyDetailedTransactions(selectedPharmacy).commissionsDue.map(tx => (
                        <tr key={tx.id}>
                          <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-900">{tx.id}</td>
                          <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">{tx.orderId}</td>
                          <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">{tx.date}</td>
                          <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">{tx.commissionMonth}</td>
                          <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">{tx.amount.toFixed(2)}</td>
                          <td className="px-4 py-2 whitespace-nowrap text-sm">
                            <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                              tx.commissionStatus === 'Paid' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                            }`}>
                              {tx.commissionStatus}
                            </span>
                          </td>
                          <td className="px-4 py-2 whitespace-nowrap text-sm">
                            {tx.commissionStatus === 'Unpaid' ? (
                              <>
                                <button
                                  onClick={() => handleMarkCommissionStatus(tx.id, 'Paid')}
                                  className="text-green-600 hover:text-green-900 p-1 rounded-full hover:bg-green-100 transition-colors"
                                  title="Mark as Paid"
                                >
                                  <CheckCircle size={18} />
                                ...
                                <button
                                  onClick={() => handleMarkCommissionStatus(tx.id, 'Not Paid')}
                      ...
                      <tr>
                        <td colSpan="7" className="px-4 py-2 text-center text-sm text-gray-500">No commissions due from this pharmacy.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
      ...
      {isReceiptUploadModalOpen && payoutToMark && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white p-8 rounded-lg shadow-xl max-w-md w-full">
            <h3 className="text-2xl font-bold text-gray-800 mb-4">Upload Receipt for Payout</h3>
            <p className="text-gray-600 mb-4">Confirming payout for ID: <span className="font-semibold">{payoutToMark.id}</span></p>
            <div className="mb-4">
              <label htmlFor="receiptUrl" className="block text-sm font-medium text-gray-700">Receipt Image URL (Mock Upload)</label>
              <input
                type="text"
                id="receiptUrl"
            ...
            <div className="flex justify-end space-x-3">
              <button
                onClick={() => { setIsReceiptUploadModalOpen(false); setPayoutToMark(null); setReceiptUrlInput(''); }}
