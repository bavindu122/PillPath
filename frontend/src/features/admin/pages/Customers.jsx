        ...
        <div className="fixed inset-0  bg-opacity-40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white p-8 rounded-lg shadow-xl max-w-md w-full">
            ...
            <div className="space-y-2 text-gray-700">
              ...
              <p><strong>Prescriptions:</strong> {selectedCustomer.prescriptions}</p>
              <p><strong>Orders:</strong> {selectedCustomer.orders}</p>
              {selectedCustomer.status === 'Loyalty' && (
                <p><strong>Loyalty Points:</strong> {selectedCustomer.loyaltyPoints}</p>
              )}
              {selectedCustomer.status === 'Suspended' && (
                <p><strong className='text-red-600'>Suspend Reson:</strong> {selectedCustomer.suspendReason}</p>
              )}
            </div>
      ...
      {suspendPopup && (
        <div className="fixed inset-0  bg-opacity-40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
            <h2 className="text-xl font-bold text-gray-800 mb-4">Suspend User</h2>
            <p className="mb-2">Are you sure you want to suspend <strong>{suspendPopup.name}</strong>?</p>
            <textarea
            ...
            <div className="flex justify-end space-x-3">
              ...
              <button
                onClick={() => {
                  console.log(`Suspending user ${suspendPopup.name} for reason: ${suspendReason}`);
                  setSuspendPopup(null);
                  setSuspendReason('');
                }}
        ...
        <div className="fixed inset-0  bg-opacity-40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
            <h2 className="text-xl font-bold text-gray-800 mb-4">Activate User</h2>
            <p className="mb-2">Are you sure you want to activate <strong>{activatePopup.name}</strong>?</p>
            <div className="flex justify-end space-x-3">
              <button
                onClick={() => setActivatePopup(null)}
                className="px-4 py-2 bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300"
              >
                Cancel
              </button>
              <button
                onClick={() => {
                  console.log(`Activated user ${activatePopup.name}`);
                  setActivatePopup(null);
                }}
