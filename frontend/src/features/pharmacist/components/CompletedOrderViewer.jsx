...
const CompletedOrderViewer = ({
  ...
  // ✅ NEW: Determine order type from order code
  const getOrderTypeFromCode = (orderCode) => {
    if (!orderCode) return "prescription";
  ...
  // ✅ NEW: Get the appropriate image to display
  const getOrderImage = () => {
    const orderType = getOrderTypeFromCode(orderData?.orderNumber || orderData?.orderCode);
    
    // For prescription orders, show prescription image
    if (orderType === "prescription") {
      return orderData?.prescriptionImageUrl || "/src/assets/img/placeholder.jpeg";
    }
    
    // For OTC orders, show first item image or placeholder
    if (orderType === "otc") {
      // Check if there are items with images
      if (orderData?.items && orderData.items.length > 0) {
        const firstItemWithImage = orderData.items.find(item => item.imageUrl || item.image);
        if (firstItemWithImage) {
          return firstItemWithImage.imageUrl || firstItemWithImage.image;
        }
      }
      // Fallback to OTC placeholder or generic placeholder
      return orderData?.otcImageUrl || "/src/assets/img/otc-placeholder.jpeg" || "/src/assets/img/placeholder.jpeg";
    }
    
    return "/src/assets/img/placeholder.jpeg";
  };

  // ✅ NEW: Get image label based on order type
  const getImageLabel = () => {
    const orderType = getOrderTypeFromCode(orderData?.orderNumber || orderData?.orderCode);
    return orderType === "prescription" ? "Prescription Document" : "OTC Order Items";
  };
    ...
    <>
      <div className="p-6">
        <div className="space-y-6">
          ...
          {/* ✅ UPDATED: Document/Items Viewer - Shows prescription OR OTC items */}
          <div className="bg-gray-50 rounded-lg p-4 border-2 border-dashed border-gray-300">
            <div className="relative">
              <div className="bg-white rounded border shadow-sm p-4">
                {/* ✅ UPDATED: Dynamic label based on order type */}
                <div className="mb-2 text-sm font-medium text-gray-600">
                  {getImageLabel()}
                </div>
                
                {/* ✅ UPDATED: Image - Shows prescription or OTC item image */}
      ...
      {/* ✅ UPDATED: Full Size Modal - Shows correct image */}
      {showModal &&
        createPortal(
