...
const CompletedOrderViewer = ({ orderData, onPrint, onExportPDF, onRefresh }) => {
  ...
  // Status helpers
  const statusOrder = ["RECEIVED", "PREPARING", "READY_FOR_PICKUP", "HANDED_OVER"];
  ...
  const humanize = (s) => s.replaceAll("_", " ").toLowerCase().replace(/(^|\s)\S/g, (c) => c.toUpperCase());
            ...
            <div className="flex items-center justify-between flex-col sm:flex-row gap-3">
              ...
              <div className="flex items-center gap-2 flex-1">
                {statusOrder.map((s, idx) => {
                  ...
                  const base = `inline-flex items-center justify-center w-8 h-8 rounded-full border ${statusColors(s)} ${isActive ? 'ring-2 ring-offset-2 ring-blue-300' : ''}`;
                  return (
                    <div key={s} className="flex items-center gap-2">
                      ...
                      {idx < statusOrder.length - 1 && (
                        <div className={`h-0.5 w-8 sm:w-12 ${isDone ? 'bg-green-300' : 'bg-gray-200'}`}></div>
                      )}
                    </div>
                  );
                })}
